...
def download_zipfile_and_create_df(year):
    # try:
        ...
        df_trh_kladn = pd.read_excel(filename_xlsx, sheet_name='Trh s NFL +', header=0, skiprows=5) # Trh s NFL +
        df_trh_zapor = pd.read_excel(filename_xlsx, sheet_name='Trh s NFL -', header=0, skiprows=5) # Trh s NFL +
        # else:
        #     filename_xlsx = f'./Rocni_zprava_o_trhu_{year}_V2_plyn.xls'
        #     df_trh_kladn = pd.read_excel(filename_xlsx, sheet_name='Trh s NFL +', header=0, skiprows=4) # Trh s NFL +
        #     df_trh_zapor = pd.read_excel(filename_xlsx, sheet_name='Trh s NFL -', header=0, skiprows=4) # Trh s NFL +
...
if __name__ == '__main__':
    ...
    date_to = pd.to_datetime('2024-12-22')
