...
from processing.get_coordinates_data import run_map_df
...
def get_openmeteo(df, start_date, end_date):
	...
	logging.info(f"Processing {len(df)} okresy...")
...
if __name__ == "__main__":
	...
	if load_map_df:
		...
		df_okresy = pd.read_csv('input_data/map_data.csv', sep=';')
	else:
		draw_maps = True
		df_okresy = run_map_df(zkz_start_date, zkz_end_date, draw_maps)

	start_date = pd.to_datetime('2010-01-01').normalize()
	end_date = pd.to_datetime('2024-12-22').normalize() # + timedelta(days=6)
	...
	if load_OpenMeteo_data:
		...
		weather_data = get_openmeteo(df_okresy, start_date, end_date)

	# print(weather_data)

	# get weights based on consumption
	df_okresy.drop('geometry', axis=1, inplace=True)
	...
	# Save to CSV
	if save_to_csv:
		file_name = 'output_data/OpenMeteo_train_data.csv'
	...
	if save_to_csv:
		file_name_0 = 'output_data/OpenMeteo_train_data_0_24.csv'
	...
	if save_to_csv:
		file_name_6 = 'output_data/OpenMeteo_train_data_6_6.csv'
